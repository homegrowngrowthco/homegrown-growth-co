...
- **Ian's remaining to-do (all his action, ~1 hr of forms):** (1) **HubSpot Solutions Directory** [highest value: relevant backlink + Solutions Partner on-ramp], (2) **Clutch**, (3) **resubmit Twilio TFV** for +18663694940 [code shipped S12], (4) **G2**, (5) add the **3 Calendly intake questions**, (6) **Netlify form-notification email** for the `free-audit` form (one-time, like sms-opt-in). Parked until material exists: **testimonials** (biggest conversion gap) + **one real case study** for `/case-studies`.
